  ...
  if (loading) {
    return (
      <section className="py-20 px-4 relative overflow-hidden">
        ...
        <div className="absolute top-0 left-0 right-0 h-[2px] bg-gradient-to-r from-transparent via-violet-500 to-transparent opacity-75 blur-sm z-0" />
        ...
        <div className="max-w-6xl mx-auto relative z-10">
          <div className="text-center">
            ...
            />
            <p className="text-gray-600 dark:text-gray-400">Fetching live GitHub data...</p>
          </div>
        </div>
      </section>
    )
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        ...
        >
          <h2 className="text-4xl md:text-5xl font-bold mb-4">
            Live <span className="bg-gradient-to-r from-purple-400 to-pink-400 bg-clip-text text-transparent">Stats</span>
          </h2>
        ...
        <div className="mb-12">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
            {statCards.map((stat, index) => (
              <motion.div
                key={index}
                className="relative bg-slate-100 dark:bg-slate-800/50 backdrop-blur-sm rounded-xl p-6 border border-purple-500/20 hover:border-purple-500/50 transition-all group overflow-hidden"
                initial={{ opacity: 0, y: 30 }}
                whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true }}
                transition={{ delay: index * 0.05 }}
                whileHover={{ y: -8, scale: 1.02, boxShadow: '0 8px 30px rgba(168, 85, 247, 0.3)' }}
              >
                {/* Gradient Background */}
        ...
        <div className="mb-12">
          <div className="flex items-center gap-3 mb-6">
            ...
            <span className="text-xs px-3 py-1 bg-blue-500/10 text-blue-500 rounded-full border border-blue-500/30">
              Estimated
            </span>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
            {linkedinStats.map((stat, index) => (
              <motion.div
                key={index}
                className="relative bg-slate-100 dark:bg-slate-800/50 backdrop-blur-sm rounded-xl p-6 border border-blue-500/20 hover:border-blue-500/50 transition-all group overflow-hidden"
                initial={{ opacity: 0, y: 30 }}
                whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true }}
                transition={{ delay: 0.4 + index * 0.05 }}
                whileHover={{ y: -8, scale: 1.02, boxShadow: '0 8px 30px rgba(96, 165, 250, 0.3)' }}
              >
                {/* Gradient Background */}
                <div className={`absolute inset-0 bg-gradient-to-br ${stat.color} opacity-0 group-hover:opacity-10 transition-opacity duration-500`} />
        ...
        {githubData?.topLanguages && (
          <motion.div
            className="bg-slate-100 dark:bg-slate-800/50 backdrop-blur-sm rounded-2xl p-8 border border-purple-500/20 mb-8"
            initial={{ opacity: 0, y: 30 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
          ...
          >
            ...
            <div className="flex flex-wrap gap-3">
              {githubData.topLanguages.map((lang, index) => (
                <motion.span
                  key={lang}
                  className="px-4 py-2 bg-gradient-to-r from-purple-500/10 to-pink-500/10 border border-purple-500/30 rounded-full text-purple-400 font-semibold"
            ...
            <div className="space-y-3">
              {githubData.recentRepos.map((repo, index) => (
                <motion.a
                  ...
                  className="block p-4 bg-slate-400/20 dark:bg-slate-700/30 rounded-lg hover:bg-purple-500/10 transition-all group"
        ...
        >
          ...
          <motion.button
            onClick={fetchGitHubData}
            className="mt-4 px-6 py-2 bg-purple-600 hover:bg-purple-700 rounded-lg text-sm font-semibold transition-colors"
            whileHover={{ scale: 1.05 }}
            whileTap={{ scale: 0.95 }}
          >
            🔄 Refresh Data
